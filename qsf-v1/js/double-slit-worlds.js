...
function precomputePhiArrayWithFixedR( x0, y0, t, r, nPhi, nProb) {
   ...
   for (let i = 0; i < nPhi; i++) {
      ...
      // Bohmian flux weighting: J·n ∝ |psi|^2 * v_r, where
      // v_r is the velocity component along the radial normal
      // of the semicircle. This approximates the correct
      // crossing probability through this surface.
   ...
   // Step 2: Choose flux-based probabilities when well-defined;
   // otherwise fall back to |psi|^2-only weighting to avoid
   // numerical pathologies near nodes.
   let normalizedProbabilities;
   if (totalFluxProb > 0 && isFinite(totalFluxProb)) {
      normalizedProbabilities = fluxProbabilities.map(prob => prob / totalFluxProb);
   } else if (totalPsi2Prob > 0 && isFinite(totalPsi2Prob)) {
      normalizedProbabilities = psi2Probabilities.map(prob => prob / totalPsi2Prob);
   } else {
      // Extreme edge case: give a uniform distribution.
      const uniformProb = 1.0 / nPhi;
      normalizedProbabilities = new Array(nPhi).fill(uniformProb);
   }
...
function restoreSimulationState() {
  ...
  if (state.interpretation) {
    ...
    $(".interp-btn").removeClass("active");
    $(`.interp-btn[data-mode="${interpretation}"]`).addClass("active");
  }
}
//==============================================================================================================
//
...
function setupGeo() {
   ...
   if      ( particleType == "neutron"  )  $('#waveSpeed').text(parseFloat((waveSpeed*1000).toFixed(3))+" mm/us");
   else if ( particleType == "electron" )  $('#waveSpeed').text(parseFloat(waveSpeed.toFixed(3))+" mm/ns");
...
async function updateSimulationState() {
   ...
   if (reset === 1) {
      ...
      $("#worldCountDisplay").text(`Branches: 1`);
   }
}
//==================================================================================================================
//
...
async function evolveParticles() {
   ...
   if ( !$('#wave-continous').prop('checked') ) {
      r1SqMin=Infinity;
      r1SqMax=0;
      r2SqMin=Infinity;
      r2SqMax=0;
   ...
   for (var iPart = 0; iPart < trajectories.length ; iPart++) {
      ...
      if ( xn > detectorXWorld ) {
         ...
         if ( iBin < 0 || iBin >= nDetectorPixels ) {
            console.log("PPPPRRROBLEM out of range iBin ", iBin);
         }
         nHits++;
         logNWorlds=logNWorlds+logNDetectorPixels;
         $("#worldCountDisplay").text(`Branches: 10^${logNWorlds.toFixed(0)}`);
...
async function renderDetectorAndHistogram() {
   ...
   if ( nHits > 0 && ( $("#plot_hits").is(':checked') > 0 || $("#plot_sensor").is(':checked') > 0 ) ) {
      ...
      for (var i = 0; i < hits.length; i++) {
         //var yHit = (nDetectorPixels - i) / yToBinCanvas; // Position on the canvas
...
function resampleHitsFromPsi() {
  ...
  for (let i = 0; i < nHits; i++) {
    if ( Math.random() < 0.5 ) detectorArray=detectorArray1
    else                       detectorArray=detectorArray2

    var randomIndex = Math.floor(Math.random() * detectorArray.length);
    yn = detectorArray[randomIndex];
    var iBin = parseInt(yn * yToBinWorld + 0.5);
    if ( iBin < 0 || iBin >= nDetectorPixels ) {
         console.log("PPPPRRROBLEM out of range iBin ", iBin);
    }
    hits[iBin]++;
...
async function drawSystem(cycleIndex) {
   ...
   let waveContinous = $('#wave-continous').prop('checked');
   if ( lastCycleIndex != cycleIndex || !waveContinous ) await renderWaveFunction(cycleIndex);
  ...
  // Re-add QPotential if missing
    if ($("#waveFunctionOption option[value='QPotential']").length === 0) {
      $("#waveFunctionOption").append(
        $("<option>", { value: "QPotential", text: "Quantum Potential" })
      );
    }
  } 
...
// Initial styling
      ...
      $(".interp-btn").removeClass("active");
      $(`.interp-btn[data-mode="${defaultMode}"]`).addClass("active");
      // Set interpretation variable and update display
      interpretation = defaultMode;
      logNDetectorPixels=Math.log10(nDetectorPixels);
      $("#worldFactor").text(`Increase in #Branches per Hit: x${nDetectorPixels}`);
      if (interpretation === "manyworlds") $("#manyWorldsInfo").show();
      else $("#manyWorldsInfo").hide();
      $("#psiTabs").tabs();
      $("#superContainer").tabs();
     ...
     $("#plot_particles, #plot_trajectories").on("change", function() {
      ...
      $('#MaxPart-units').hide();
      $('#WaveWidth-units').hide();
      $('#det-pixels-units').hide();
