        ...
        {canvasTab === "about" && (
          <div style={{ flex:1, overflowY:"auto", padding:"28px 36px",
            ...
            {[
              ...
              "At the barrier, the wavefunction splits into two branches. The transmitted branch moves to the upper right — particle goes through, pointer deflects up. The reflected branch moves to the lower left — particle bounces back, pointer deflects down. The marginal projections on the sides show two distinct peaks emerging, one for each outcome.",
              "What happens next depends on your interpretation of quantum mechanics.",
              "In the Copenhagen interpretation, measurement causes the wavefunction to collapse. One branch survives; the other vanishes. The outcome is random, governed by the Born rule: the probability of each outcome equals the squared norm of the corresponding projected state. The pointer lands at a definite value — this is what we observe in the lab.",
              "The Copenhagen interpretation offers two levels of description. Switch to Operator view to see the textbook formalism without the apparatus. The observable is the Hermitian operator M̂ = (+1)Π̂_T + (−1)Π̂_R, where Π̂_T and Π̂_R are the projectors onto the transmitted and reflected subspaces. Before measurement the state is |ψ⟩ = √T |ψ_T⟩ + √R |ψ_R⟩. Applying the projector gives Π̂_T|ψ⟩ = √T|ψ_T⟩, with probability P(m=+1) = ‖Π̂_T|ψ⟩‖² = T. The post-measurement state is the renormalised projection: |ψ_post⟩ = |ψ_T⟩, which satisfies the eigenvalue equation M̂|ψ_T⟩ = +1·|ψ_T⟩. Use the Step button to walk through these stages one click at a time.",
              "In the Pilot-Wave interpretation — also called Bohmian mechanics — the wavefunction never collapses. Both branches continue to exist. But the particle follows a definite trajectory, guided by the wave. The white dot traces that path. It enters one branch and stays there, guided deterministically by its initial position. The other branch becomes empty: it still exists mathematically, but carries no particle and has no further physical effect. This is the empty wave.",
              ...
              "Both interpretations give identical experimental predictions. The difference is not in what we measure — it is in what we believe is really happening.",
            ].map((para, i) => (
              <p key={i} style={{
                marginBottom:"1.2em",
