...
export default function App() {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      const yT       = yRFixed + 2 * effLam * dtPShown, yR = yRFixed;
      ...
      td.forEach(({ pts, isTransmit }, i) => {
        ...
        if (fl.rawY && fl.dtAs) {
          ...
          for (let j = 0; j <= STEPS; j++) {
            ...
            const branchShift = isTransmit ? (2 * effLam * dtP_j) : 0;
        ...
        const branchShiftCur = isTransmit ? (2 * effLam * dtP_cur) : 0;
      ...
      if (isMW) {
        ...
        const sepA     = yRFixed + effLam * dtA;
        const sepSlope = effLam > 0 ? -s.k0 / (2 * effLam) : 0;
      ...
      if (Tr.gaugeSprite && s.detectorOn) {
        ...
        if (!pointerHit) {
          ...
          if (!s.colTriggered) {
            ...
            // Collapsed to T: winner at full T deflection (fraction 1)
            needles.push({ fraction: 1,  color: "#22ee88", alpha: 0.95 });
            needles.push({ fraction: 0,  color: "#ff7744", alpha: (1 - s.colFade) * 0.55 });
          } else {
            // Collapsed to R: winner stays at rest (fraction 0), T ghost fades from meanFrac
            needles.push({ fraction: 0,        color: "#ff7744", alpha: 0.95 });
            needles.push({ fraction: meanFrac, color: "#22ee88", alpha: (1 - s.colFade) * 0.55 });
          }
        }
