...
void main(){
  ...
  float t = sqrt(clamp(rho * ${RHO_SCALE}, 0.0, 1.0));
  vec3 col = inferno(t);
  float px = uv.x * ${N}.0;
...
function buildGPUEngine(gl) {
  ...
  function display(W, H, sb, sq, barX0, barX1, qX0, qX1) {
  ...
  function reset(barrierOn, v0eV=V0_DEFAULT, widthPx=WIDTH_DEFAULT) {
    upload(pA, buildInitialPsi());
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (c.barrierChanged) {
        ...
        eng.reset(c.barrierOn, c.barrierV0eV, c.barrierWidthPx);
